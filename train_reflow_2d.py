...
def main():
    ...
    parser.add_argument("--pretrained-model", type=str, required=True, help="Path to the pretrained CFM model.")
    parser.add_argument("--output-dir", type=str, default="outputs")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    set_seed(42)
    args.output_dir = Path(args.output_dir) / "reflow" / args.dataset
    ...
    learning_rate = 1e-3
    batch_size = 4096
    iterations = 2000  # we only train 1/10 of the steps in training CFM
    log_every = 200
    hidden_dim = 512
    ...
    pretrained_flow = Mlp(dim=dataset.dim, time_dim=1, h=hidden_dim).to(device)
    ...
    optimizer = torch.optim.AdamW(reflow.parameters(), learning_rate)

    class WrappedModel(ModelWrapper):
        def forward(self, x: Tensor, t: Tensor, **extras) -> Tensor:
            return self.model(x_t=x, t=t)

    pretrained_flow = WrappedModel(pretrained_flow)

    # Training

    losses = []
    for global_step in range(iterations):
        x_0 = torch.randn([batch_size, *data_shape], dtype=torch.float32, device=device)
        t = torch.rand(batch_size, 1, dtype=torch.float32, device=device)

        x_1 = integrate_source_samples(pretrained_flow, x_0)
    ...
    reflow.eval()
    torch.save(reflow.state_dict(), Path(args.output_dir) / "ckpt.pth")
